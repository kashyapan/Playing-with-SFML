...
int main()
{
    ...
    if(!foodTexture.loadFromFile("./images/green.png"))
    {
        std::cout << "Loading Image Failed" <<"\n" ;
    }
...
//                std::cout <<"key pressed "<<event.key.code<<"\n" ;
                ...
                if(isIntersecting(snake.front().getPosition() , food.getPosition()))
                {
                    growSnake(snake);
                    putFood(food , window.getSize() );
                }
                ...
                //Snake Bite
                //If head isInersecting with any part of body then
                //bodysize = head to intersecting part
                handleSnakeBite(snake ,headPosition) ;
